...
//     // Fetch all cities and filter them based on the search term
//     let data = await CityModel.find(); // Fetch all cities from DB

//     // Map over the data to convert Mongoose documents to plain JS objects, then filter based on name
//     let filteredCities = data
//       .map((city) => city.toObject()) // Convert to plain JavaScript object
//       .filter((city) => city.name.toUpperCase().includes(q)); // Perform the filtering

//     // If no cities match the query, return an empty array or a message
//     if (filteredCities.length === 0) {
//       return res
//         .status(404)
//         .send({ status: "failed", data: "No cities found" });
//     }
//     // Return the filtered cities
//     return res.send({ status: "success", data: filteredCities });
//   } catch (error) {
//     // If there's an error, send a 500 status code with the error message
//     return res.status(500).send({ status: "failed", data: error.message });
//   }
